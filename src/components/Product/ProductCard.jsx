      ...
      <div className="card flex flex-col sm:flex-row">
        <Link to={`/product/${product.id}`} className="sm:w-48 h-48 flex-shrink-0 relative">
          ...
          {isOutOfStock && (
            <div className="absolute inset-0 bg-black/70 flex items-center justify-center rounded-l-xl">
              <span className="text-red-400 font-bold text-sm px-2 py-1 bg-red-500/20 rounded-lg">OUT OF STOCK</span>
            </div>
          )}
        </Link>
        <div className="p-4 flex-1">
          ...
          <div className="flex items-center justify-between mt-4">
            ...
            {isOutOfStock ? (
              <div className="bg-red-500/20 text-red-400 px-4 py-2 rounded-lg flex items-center space-x-2">
    ...
    <div className="card group">
        ...
        <div className="relative overflow-hidden h-64">
          ...
          {isOutOfStock && (
            <div className="absolute inset-0 bg-black/70 flex items-center justify-center">
              <span className="text-red-400 font-bold text-lg px-3 py-2 bg-red-500/20 rounded-lg">OUT OF STOCK</span>
            </div>
          )}
        </div>
      </Link>
      <div className="p-4">
        ...
        <div className="flex space-x-2">
          ...
          <Link to={`/product/${product.id}`}>
            <button className="p-2 border border-gray-700 rounded-lg hover:border-primary-500 hover:text-primary-400 transition-colors text-gray-400">
              <FiEye size={18} />
            </button>
          </Link>
        </div>
